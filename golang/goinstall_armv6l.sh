...
do
    ...
    case $choose in
        1)
            ...
            gosrc=$(ls | grep linux-armv6l.tar.gz | sed -n '1p')
        ...
        2)
            ...
            new_version=$(curl -s  https://studygolang.com/dl |  sed -n '/dl\/golang\/go.*\.linux-armv6l\.tar\.gz/p' | sed -n '1p' | sed -n '/1/p' | awk 'BEGIN{FS="\""}{print $4}' | awk 'BEGIN{FS="/"}{print $4}' | tr -cd "[0-9]" )
